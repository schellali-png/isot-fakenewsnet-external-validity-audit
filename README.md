# Benchmark-Validity Audit for ISOT and FakeNewsNet

Version `1.1.0` — 2026-07-31

This public-release package accompanies the manuscript *Beyond In-Corpus
Accuracy: A Reproducible Audit of Leakage, Source-Label Association, and
Cross-Dataset Transfer in Fake-News Classification*. It contains executable
analysis workflows, protocol records, aggregate results, and text-free
item-level predictions. It does **not** redistribute the six original
ISOT/FakeNewsNet CSV files.
...
v1.0.0 and v1.1.0 were requested after review and are explicitly treated as post hoc
robustness evidence rather than preregistered confirmation.

## Complete v1.1.0 main-analysis workflows

Version 1.1.0 adds the three top-level workflows needed to regenerate the
central internal, temporal, and calibration analyses from the original CSV
files. Each workflow records input hashes, cleaning counts, parameters, and
software versions; item-level outputs contain identifiers and hashes but no
redistributed article or title text.

- `scripts/group_aware_internal_validation.py` reconstructs combined-text
  exact/Jaccard-0.80 components, creates stratified group folds, evaluates the
  four fixed TF-IDF models on full text, Reuters-masked full text, and titles,
  and emits the conventional holdout, source-rule comparison, inherited title
  folds, and Linear-SVM feature table.
- `scripts/chronological_evaluation.py` parses ISOT dates, trains on rows before
  a declared cutoff, tests on later rows, and removes every near-duplicate
  component that crosses the boundary. It reports fixed-model, Reuters-rule,
  Reuters-masked-SVM, quarterly, paired, and date-parsing audits.
- `scripts/calibration_transfer.py` performs nested group-fold Platt
  calibration inside ISOT and transfers a mapping fitted only to ISOT
  out-of-fold margins to PolitiFact. It reports Brier score, log loss,
  equal-width ECE, reliability bins, and selective-prediction coverage without
  fitting on any PolitiFact label.

The nested calibration design is deliberately stricter than fitting a
calibrator to margins from the same records on which it is evaluated. Treat
its generated calibration tables as the source of truth for the revised
manuscript and update any earlier non-nested table rather than expecting exact
agreement. The original datasets are excluded, so the new result tables are
generated locally rather than bundled as unverifiable precomputed outputs.
...
- `scripts/group_aware_internal_validation.py`: combined-text group
  reconstruction, inherited title folds, fixed-model internal validation,
  source-rule diagnostics, and text-free predictions.
- `scripts/chronological_evaluation.py`: date parsing, cross-cutoff component
  removal, future-period evaluation, quarterly metrics, and paired tests.
- `scripts/calibration_transfer.py`: nested ISOT calibration, locked
  ISOT-to-PolitiFact probability transfer, reliability bins, and selective
  prediction.
...
- `tests/smoke_new_workflows.py`: synthetic, text-safe end-to-end smoke test for
  all three v1.1.0 workflows.
...
Run the three v1.1.0 main-analysis workflows with:

```bash
python scripts/group_aware_internal_validation.py \
  --isot-fake /path/to/Fake.csv \
  --isot-true /path/to/True.csv \
  --output reproduced_group_validation \
  --models nb,lr,svm,rf \
  --strict-manuscript-snapshot

python scripts/chronological_evaluation.py \
  --isot-fake /path/to/Fake.csv \
  --isot-true /path/to/True.csv \
  --output reproduced_chronological \
  --cutoff 2017-01-01 \
  --models nb,lr,svm,rf \
  --strict-manuscript-snapshot
...
python scripts/calibration_transfer.py \
  ...
  --politifact-fake /path/to/politifact_fake.csv \
  --politifact-real /path/to/politifact_real.csv \
  --fold-assignments reproduced_group_validation/isot_title_inherited_group_fold_assignments.csv \
  --output reproduced_calibration \
  --strict-manuscript-snapshot
```

The strict switches verify the manuscript snapshot counts and stop on a data
or preprocessing mismatch. Without those switches, the scripts can be used on
compatible snapshots while still recording the observed counts. Run the
packaged smoke test independently with `python tests/smoke_new_workflows.py`.
